...
if Config.EnableXtoCancel then
    RegisterKeyMapping("emotecancel", "Cancel current emote", "keyboard", "F6")
end
...
function DestroyAllProps()
    ...
    for _,v in pairs(PlayerProps) do
        if DoesEntityExist(v) then
            TriggerServerEvent("DeleteObject",ObjToNet(v))
        end
    end
    PlayerHasProp = false
...
function AddPropToPlayer(prop1, bone, off1, off2, off3, rot1, rot2, rot3)
    ...
    -- prop = CreateObject(joaat(prop1), x, y, z + 0.2, false, false, true)
    -- AttachEntityToEntity(prop, Player, GetPedBoneIndex(Player, bone), off1, off2, off3, rot1, rot2, rot3, true, true,
    -- false, true, 1, true)

    local Progression,Network = vRPS.CreateObject(prop1,x,y,z)
    if Progression then
        prop = LoadNetwork(Network)
