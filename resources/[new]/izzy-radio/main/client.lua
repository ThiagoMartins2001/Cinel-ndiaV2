...
RegisterNUICallback('createChannel', function(data, cb)
    local created = Execute.createChannel(data)
...
RegisterNUICallback('disconnect', function(data, cb)
    local disconnect = Execute.disconnect()
...
RegisterNUICallback('connect', function(data, cb)
    if string.len(tostring(data.frequency)) > 3 then TriggerEvent("Notify", 'negado', 'Você deve se conectar em uma frequencia igual ou inferior a 999.', 5000) return end
    local available = isChannelSpecial(data.frequency)

    if available then
    local fetch = Execute.connect(data)
...
RegisterNUICallback('password', function(data, cb)
    local password = Execute.password(data)
    local available = isChannelSpecial(data.frequency)
...
RegisterNUICallback('favorite', function(data, cb)
    local fetch = Execute.favorite(data, lastPassword)
...
RegisterNUICallback('volume', function(data, cb)
    if data.value then
        ...
        if volume < 1 then
            ...
            volume = volume - 10
            notify('volume_down', "para: "..volume)
        end
    end
...
function refreshMemebers()
    fetch = Execute.getMembers(targetFrequency)
