...
Citizen.CreateThread(function()
    zof.prepare("zo/attachs_createtable",[[CREATE TABLE IF NOT EXISTS zo_attachs(user_id INTEGER,attachs text, PRIMARY KEY (`user_id`) USING BTREE)]])
...
src.checkPayment = function(price, gun)
    ...
    if price then
        return zof.paymentFull(user_id, price)
    end
    return true
end
...
src.checkContainsAttachs = function(attachs, gun)
    ...
    if todosAttachs then
        for i, component in pairs(attachs) do
            zof.TakeItem(user_id, attachsItens[component.type], 1)
        end
    end
    return todosAttachs
end
...
AddEventHandler("CharacterChosen",function(user_id, source, first_spawn)
...
AddEventHandler("playerDropped",function(reason)
    local source = source
    local user_id = vRP.Passport(source)
    if cacheAttachsPlayers[user_id] == nil then return end
