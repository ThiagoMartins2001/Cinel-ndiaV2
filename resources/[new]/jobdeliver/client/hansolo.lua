...
Tunnel.getInterface("zGarages", vSERVER)
Tunnel.bindInterface("jobdeliver", Hiro)
Proxy.addInterface("jobdeliver", Hiro)
Hiro = Tunnel.getInterface("jobdeliver")
--------------------------------------------------------------------------------------------------------------------------------------------
----VARIAVEIS
--------------------------------------------------------------------------------------------------------------------------------------------
local x,y,z = -70.86,16.62,72.39
...
--------------------------------------------------------------------------------------------------------------------------------------------                                                         
local restaurantes = {
	[1] = {x=-73.99,y=24.04,z=72.94}, -- Frango 1
	[2] = {x=-73.99,y=24.04,z=72.94}, -- Hamburger
	[3] = {x=-73.99,y=24.04,z=72.94}, -- Hamburguer 2
	[4] = {x=-73.99,y=24.04,z=72.94}, -- Chilli
	[5] = {x=-73.99,y=24.04,z=72.94}, -- Macarrao
	[6] = {x=-73.99,y=24.04,z=72.94}, -- Sanduiche      
	[7] = {x=-73.99,y=24.04,z=72.94}, -- Sanduiche2
	[8] = {x=-73.99,y=24.04,z=72.94}, -- Tequila
	[9] = {x=-73.99,y=24.04,z=72.94}, -- Hambrgue gurme
	[10] = {x=-73.99,y=24.04,z=72.94}, -- Pizza
	[11] = {x=-73.99,y=24.04,z=72.94}, -- Comida chinesa
	[12] = {x=-73.99,y=24.04,z=72.94}, -- Muffin
	[13] = {x=-73.99,y=24.04,z=72.94}, -- Taco
	[14] = {x=-73.99,y=24.04,z=72.94}, -- Café
	[15] = {x=-73.99,y=24.04,z=72.94} -- Comida chinesa
}
--------------------------------------------------------------------------------------------------------------------------------------------
----ENTREGAS
--------------------------------------------------------------------------------------------------------------------------------------------
...
-- 		nveh = CreateVehicle(mhash,77.82,265.48,109.24,164.41+0.5,0.86,true,false)
-- 		SetVehicleIsStolen(nveh,false)
-- 		SetVehicleOnGroundProperly(nveh)
-- 		SetEntityInvincible(nveh,false)
-- 		SetVehicleNumberPlateText(nveh)
-- 		InvokeNative(0xAD738C3085FE7E11,nveh,true,true)
			...
			if trabalhando and pedido then
				...
				if distance <= 50 then
					...
					if IsControlJustPressed(0,101) then
						TriggerEvent("Notify","verde", "Aguarde enquanto preparam o pedido para entrega",4000)
							RemoveBlip(blip)  -- remove o blip anterior
							Wait(4000)  
							TriggerEvent("Notify","verde","Pedido recebido, entregue ao cliente marcado em seu GPS",8000)
			...
			if trabalhando and entrega then
				...
				if distance <= 50 then
					...
					if distance < 3 then
						...
						if IsControlJustPressed(0,101) then
							TriggerEvent("Notify","verde","Aguarde o cliente no local",8000)
							...
							local lanche = CreateObject(GetHashKey("prop_paper_bag_small"), clientecoord.x, clientecoord.y, clientecoord.z, 1, 1, 1)
							sacolaLanche = lanche
							...
							local distancia = GetDistanceBetweenCoords(restaurantes[destino].x, restaurantes[destino].y, restaurantes[destino].z, entregas[destinoEntrega].x,entregas[destinoEntrega].y,entregas[destinoEntrega].z,true)
							local payment = Hiro.Payment(distancia) --#CALCULA E REALIZA payment delivery
							qtEntrega = qtEntrega + 1
							dinheiroGanhoEntrega = dinheiroGanhoEntrega + payment
...
RegisterNetEvent('jobdeliver:StartJob')
AddEventHandler('jobdeliver:StartJob', function()
	AtualizaHUD(0,0,"Restaurante")
	local ped = PlayerPedId()
	if not trabalhando and not motodelivery then
		...
		trabalhando = true          
		pedido = true
		TriggerEvent("Notify", "verde", "Você iniciou o expediente, bom serviço.",8000)
		openGui()
		destino = math.random(1,15)		
		distanciaDestino = calculaDistancia(restaurantes[destino].x,restaurantes[destino].y,restaurantes[destino].z)
		while distanciaDestino > 500 do
	...
	elseif trabalhando then
		...
		motodelivery = false
		RemoveBlip(blip)
		TriggerEvent("Notify", "amarelo", "Você encerrou seu expediente, obrigado por trabalhar para o delivery",8000)
	end
end)
