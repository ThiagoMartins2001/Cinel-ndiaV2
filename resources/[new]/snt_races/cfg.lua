...
execute = function(key,    data)
    return vRP.execute(key,    data, "execute")
end
...
remove_race_ticket = function(user_id)
    if vRP.TakeItem(user_id,    "races",    1,     true) then
        return true
    end
    TriggerClientEvent("Notify",    vRP.Source(user_id),     "vermelho",    "Você precisa de um Ticket de Corrida!",    10000)
...
notify_function = function(source,    type,    message) -- função de notificação
    if type == "error" then
        TriggerClientEvent("Notify",    source,     "vermelho",    message,    10000)
    else
...
pay_race_prize = function(user_id,    prize)
    local cops = vRP.NumPermission('PoliciaMilitar')
    for k,    v in pairs(cops) do
        prize = prize + 1250
    end
    vRP.GiveItem(user_id,"dollarz",prize)
    TriggerClientEvent("Notify",    source,     "verde",    "Parabéns,     você finalizou a corrida e recebeu R$ "..format_currency(prize)..",    00",    25000)
end
...
notify_police_start = function()
    ...
    local copAmount = vRP.NumPermission('PoliciaMilitar')
    ...
    for l,    w in pairs(copAmount) do
        ...
        if player then
            async(function()
          TriggerClientEvent("NotifyPush",    player,    { code = 40,     title = "OCORRÊNCIA",     x = x,     y = y,     z = z,     badge = "CORRIDA ILEGAL" },     'corrida')
...
races_block_by_permission = {"PoliciaMilitar"}

-- localização do blip para abrir o menu / iniciar corridas | blip location to open menu/start races
explosive_races_start_point = {2351.25,     3134.47,     47.74,    80.00}
