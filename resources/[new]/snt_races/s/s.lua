Citizen.CreateThread(function()
    prepare("snt_races/downloadData","SELECT * FROM snt_races")
    prepare("snt_races/purgeData","DELETE FROM snt_races WHERE user_id = @user_id AND data_type = @data_type AND race_name = @race_name")
    prepare("snt_races/insertData","INSERT INTO snt_races(user_id,race_name,data_type,data_value) VALUES (@user_id,@race_name,@data_type,@data_value)")
end)
...
AddEventHandler("snt_races/srequest/start_race",function(rn)
    ...
    if r then
        if explosive_races[r].needed ~= nil then
            if vRP.TakeItem(user_id,"races",1) then
