...
RegisterNUICallback("buyComponent", function(data, cbj)
    ...
    local result = vSERVER.buyComponent({ price = data.component.price, currentMechanic = currentMechanic, plate = vehPlate}, getVehicleProperties(vehicle))
    if result then
...
RegisterNUICallback("applyFitment", function(dt, cbj)
    local vehicle = GetVehiclePedIsUsing(PlayerPedId())
    local vehPlate = trim(GetVehicleNumberPlateText(vehicle))
    local result = vSERVER.buyComponent({ price = Config.Menus.fitment.price, currentMechanic = currentMechanic, plate = vehPlate}, getVehicleProperties(vehicle))
    if result then
        if not result.status then
            return cbj(false)
        end
...
RegisterNUICallback("buyColor", function(data, cbj)
    ...
    local result = vSERVER.buyComponent({ price = colorPrice, currentMechanic = currentMechanic, plate = vehPlate}, getVehicleProperties(vehicle))
    if result then
        if not result.status then
            return cbj(false)
        end
...
RegisterNUICallback("buyBasket", function(data,cb)
    ...
    local result = vSERVER.buyBasketData({data.type, data.basketdata, currentMechanic,vehPlate}, getVehicleProperties(vehicle))
    if result then
