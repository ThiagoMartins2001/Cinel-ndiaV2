...
local Objects = {}
bancadas = {}
local ImageUrl = "http://45.146.81.222/inventory/"
local quantidadePolicia = -2
...
function Creative.sellDrug(Number)
    ...
    if vRP.Request(source, itemName(bancadas[Passport][Index].item), getRandomMessage()) then
        if bancadas[Passport][Index]["counnInTable"] >= Amount then
            ...
            vRP.GenerateItem(Passport, "dollarz", money)
                ...
                --  TriggerEvent("Wanted",source,Passport,300)
                    ...
                    for Passports,Sources in pairs(Service) do
                        async(function()
                            ...
                            TriggerClientEvent("NotifyPush",Sources,{ code = 20, title = "Denúncia Anônima", x = Coords["x"], y = Coords["y"], z = Coords["z"], criminal = "Suspeita de tráfico de drogas", time = "Recebido às "..os.date("%H:%M"), blipColor = 16 })
                        end)
