		...
		if IsPedInAnyVehicle(Ped) then
			...
			if Classes ~= 13 and Classes ~= 14 then
				...
				if GetPedInVehicleSeat(Vehicle,-1) == Ped then
					...
					if Roll > 75.0 or Roll < -75.0 and not IsEntityInAir(Vehicle) then
		...
		if IsPedInAnyVehicle(Ped) then
			...
			if GetPedInVehicleSeat(Vehicle,-1) == Ped then
				...
				if Roll > 75.0 or Roll < -75.0 then
					if math.random(100) <= 10 and GetVehicleClass(Vehicle) ~= 8 and GetVehicleClass(Vehicle) ~= 13 and GetVehicleClass(Vehicle) ~= 14 and GetVehicleClass(Vehicle) ~= 15 and GetVehicleClass(Vehicle) ~= 16 then
