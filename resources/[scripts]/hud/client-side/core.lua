				...
				if Display then
					...
					if GetEntityMaxHealth(Ped) ~= 200 then
						if Health ~= parseInt(Healing * 0.66) then
							...
							SendNUIMessage({ Action = "Health", Payload = Healing / 1 })
							Health = Healing
						end
					else
						if Healing > 100 then
...
AddEventHandler("hud:Passport",function(Value)
...
AddEventHandler("hud:Players",function(Value)
	SendNUIMessage({ Action = "Players", Payload = vRP.Passport(source) })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- HUD:VOIP
-----------------------------------------------------------------------------------------------------------------------------------------
