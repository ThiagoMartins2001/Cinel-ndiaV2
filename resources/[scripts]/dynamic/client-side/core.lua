...
RegisterCommand("PlayerFunctions",function()
	...
	if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not Dynamic and LocalPlayer["state"]["Route"] < 900000 and not IsPauseMenuActive() then
		...
		if vRP.ClosestVehicle(7) then
			if not IsPedInAnyVehicle(Ped) then
				exports["dynamic"]:AddButton("Rebocar","Colocar veículo na prancha do reboque.","towdriver:invokeTow","","",false)
...
RegisterCommand("EmergencyFunctions",function()
	if (CheckPolice() or CheckParamedic()) and not IsPauseMenuActive() then
		if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not Dynamic and LocalPlayer["state"]["Route"] < 900000 then
		local Ped = PlayerPedId()
		local Health = GetEntityHealth(Ped)

		if CheckPolice() or CheckCore() or CheckPRF() or CheckPCivil() then
			-- exports["dynamic"]:AddButton("Placa","Verificar emplacamento.","prison:Plate","",false,true)
			-- exports["dynamic"]:AddButton("Prender","Adicionar ficha criminal.","police:Mdt","",false,true)
			exports["dynamic"]:AddButton("Computador","Para acessar digite: tabletp no seu f8","police:open","",false,false)
			...
			if Health > 100 and not IsPedInAnyVehicle(Ped) then
				...
				exports["dynamic"]:AddButton("PoliciaMilitar","Fardamento de oficial.","player:Preset","1","prePolice",true)
				exports["dynamic"]:AddButton("Core","Fardamento de oficial.","player:Preset","2","prePolice",true)
				exports["dynamic"]:AddButton("PRF","Fardamento de oficial.","player:Preset","3","prePolice",true)
				exports["dynamic"]:AddButton("PCivil","Fardamento de oficial.","player:Preset","4","prePolice",true)
				exports["dynamic"]:AddButton("Judiciary","Fardamento de oficial.","player:Preset","5","prePolice",true)
				exports["dynamic"]:AddButton("CMD GERAL","Fardamento de comando geral.","player:Preset","6","prePolice",true)
				exports["dynamic"]:AddButton("GRAER","Fardamento de GRAER.","player:Preset","7","prePolice",true)
				exports["dynamic"]:AddButton("GRAER PRF","Fardamento de GRAER PRF.","player:Preset","8","prePolice",true)
				exports["dynamic"]:AddButton("GTM PRF","Fardamento de GTM PRF.","player:Preset","9","prePolice",true)
				exports["dynamic"]:AddButton("GTM","Fardamento de GTM.","player:Preset","10","prePolice",true)
				exports["dynamic"]:AddButton("SPEED","Fardamento de SPEED.","player:Preset","11","prePolice",true)
	
			end

			exports["dynamic"]:Open()
		elseif CheckParamedic() and Health > 100 and not IsPedInAnyVehicle(Ped) then
			exports["dynamic"]:AddMenu("Jogador","Pessoa mais próxima de você.","player")
			exports["dynamic"]:AddButton("Carregar","Carregar a pessoa mais próxima.","player:carryPlayer","","player",true)
			exports["dynamic"]:AddButton("Colocar no Veículo","Colocar no veículo mais próximo.","player:cvFunctions","cv","player",true)
			exports["dynamic"]:AddButton("Remover do Veículo","Remover do veículo mais próximo.","player:cvFunctions","rv","player",true)
			...
			exports["dynamic"]:AddButton("SAMU","Permitido apenas para o cargo descrito!","player:Preset","12","preMedic",true)
			exports["dynamic"]:AddButton("UPA","Permitido apenas para o cargo descrito!","player:Preset","13","preMedic",true)
			exports["dynamic"]:AddButton("CHEFE UPA","Permitido apenas para o cargo descrito!","player:Preset","14","preMedic",true)
