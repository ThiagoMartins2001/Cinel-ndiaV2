...
-----------------------------------------------------------------------------------------------------------------------------------------
-- FUNÇÃO PARA DEFINIR O TEMPO DE MORTE BASEADO EM VIP
-----------------------------------------------------------------------------------------------------------------------------------------
function UpdateDeathTime()
   -- print("🧠 UpdateDeathTime chamado no momento da morte")
    CreateThread(function()
        local vipType = TriggerServerCallback("survival:GetVipGroup")
        print("🔁 Resultado do callback VIP:", vipType)

        if vipType == "vipabsolutultimate" then
            Death["Default"] = 30
        elseif vipType == "vipabsolutultimate" then
            Death["Default"] = 60
        elseif vipType == "vipadvanced" then
            Death["Default"] = 80
        elseif vipType == "vipdiamond" then
            Death["Default"] = 100
        elseif vipType == "vipplatium" then
            Death["Default"] = 150
        elseif vipType == "vipgold" then
            Death["Default"] = 200
        else
            Death["Default"] = 300
        end

       -- print("⏱️ Death.Default agora é:", Death["Default"])
    end)
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- CALLBACK SUPORTE
		...
		if LocalPlayer["state"]["Active"] then
			...
			if GetEntityHealth(Ped) <= 100 then
				if not Death["Status"] then
					...
					--print("🧠 UpdateDeathTime chamado no momento da morte")
					UpdateDeathTime()
					--print("🧠 UpdateDeathTime foi chamado após a morte!")
					...
					if IsPedInAnyVehicle(Ped) then
						...
						if Crawl["Timer"] > 0 then
							...
							if not Crawl["Status"] then
								...
								elseif GetGameTimer() >= Crawl["Stand"] and not LocalPlayer["state"]["Rope"] and not IsEntityPlayingAnim(Ped,"amb@world_human_sunbathe@male@front@idle_a","idle_a",3) then
							...
							if not LocalPlayer["state"]["Rope"] and not IsEntityPlayingAnim(Ped,"dead","dead_a",3) then
								TaskPlayAnim(Ped,"dead","dead_a",8.0,8.0,-1,1,1,0,0,0)
							end
						end
					end

					if Death["Status"] and Death["Timer"] <= 0 and Crawl["Timer"] <= 0 and not LocalPlayer["state"]["Rope"] and IsControlPressed(0,38) then
