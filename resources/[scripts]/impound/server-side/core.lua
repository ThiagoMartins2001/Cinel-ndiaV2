...
AddEventHandler("police:Impound",function(entity)
	...
	if Passport then
		if not Impound[entity[2].."-"..entity[1]] then
			...
			vRP.SendWebhook(webhookregistro, "LOGs Registro", "**Passaporte: **"..Passport.."\n**Registrou o veículo: **"..entity[2].."-"..entity[1], 10357504)
...
RegisterCommand("placa",function(source,Message)
	local Passport = vRP.Passport(source)
	if Passport then
		if (vRP.HasService(Passport,"PoliciaMilitar") or vRP.HasService(Passport,"Core") or vRP.HasService(Passport,"PRF") or vRP.HasService(Passport,"PCivil")) and Message[1] then
			...
			if Passport then
				...
				if Vehicle[1] then
					if Vehicle[1]["arrest"] <= os.time() then
						...
						vRP.SendWebhook(webhookapreender, "LOGs Apreencao", "**Passaporte: **"..Passport["Passport"].."\n**Apreendeu o veículo: **"..Vehicle[1].vehicle, 10357504)
					else
