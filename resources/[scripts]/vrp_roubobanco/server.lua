...
function vRPrb.chamarPoliciais()
    ...
    if user_id then
        local soldado = vRP.NumPermission("Police")
        for l,w in pairs(soldado) do
            local player = vRP.Source(parseInt(w))
            if player then
                async(function()
                    TriggerClientEvent("NotifyPush",player,{ code = 31, title = "Roubo ao Banco Central", x = x, y = y, z = z, time = "Recebido às "..os.date("%H:%M"), blipColor = 22 })
...
function vRPrb.updateCops(message)
    ...
    local user_id = vRP.Passport(source)
    if user_id then
        TriggerClientEvent('chatMessage', -1, "[NEWS BANCO CENTRAL]", {64,64,255}, message)
    end
end


function vRPrb.copsonline()
	local cops = vRP.NumPermission("Police")
	return #cops 
end
...
function vRPrb.getitem(item)
    local source = source
	local user_id = vRP.Passport(source)
    if vRP.TakeItem(user_id,item,1,true) then
		return true
    else
		return false
	end
...
function generateRandomMoney(src, bank)
    ...
    while true do
        ...
        if Config.BankRobbery[bank].Money.Amount - randommoney >= 0 then
			...
			vRP.GenerateItem(xPlayer,"dollarz",randommoney,true)
            break
        end
        Wait(0)
    end
...
AddEventHandler('vrp_roubobanco:takeMoney', function(bank)
    ...
    if Config.BankRobbery[bank].Money.Amount - 1000 >= 0 then
        ...
        if Config.BankRobbery[bank].Money.Amount > 0 then
			vRP.GenerateItem(xPlayer,"dollarz",Config.BankRobbery[bank].Money.Amount,true)
...
AddEventHandler('vrp_roubobanco:lootSafe', function(bank, safe)
	...
	vRP.GenerateItem(xPlayer,"dollarz",randommoney,false)
	TriggerClientEvent("Notify",src,'verde',"Você encontrou R$".. randommoney .."!", 5000)
