	...
	$(".populated").tooltip({
		create: function(event,ui){
			...
			if (name == "Passaporte" || name == "Distintivo"){
				...
				contents = `<item>${name} - ${Passport}</item>${description !== "false" ? "<br><description>"+description+"</description>":""}<br><legenda>Nome: <r>${idName}</r><br>Tipo Sangüineo: <r>${idBlood}</r><br>Vagas na Garagem: <r>${Garage}</r><br>Validade: <r>${idVality}</r><br>Visto: <r>${Visa}</r><br>Origem do Documento: <r>${Locate}</r><br>Diamantes: <r>${Gems}</r><br>Premium: <r>${idPremium}</r></legenda>`;
			}
