...
RegisterCommand("debugchest", function()
    ...
    if Chests then
        print("✅ Chests encontrado com " .. #Chests .. " baús")
        for Name, v in pairs(Chests) do
        ...
        print("❌ Chests não encontrado")
    end
end, false)

-- DEBUG ESPECÍFICO PARA PRF
RegisterCommand("debugprf", function()
    ...
    if Chests then
        print("Total de baús: " .. #Chests)
        for i, v in pairs(Chests) do
            if v["Name"] == "PRF" then
                local distance = #(playerCoords - v["Coords"])
                print("🎯 Baú PRF encontrado!")
                print("Coordenadas do baú: " .. v["Coords"].x .. ", " .. v["Coords"].y .. ", " .. v["Coords"].z)
                print("Distância: " .. distance .. " metros")
                print("Mode: " .. v["Mode"])
                
                if distance < 10.0 then
                    print("✅ Baú PRF está próximo!")
                else
                    print("❌ Baú PRF está longe")
                end
                return
            end
        end
        print("❌ Baú PRF não encontrado na lista")
        print("Baús disponíveis:")
        for i, v in pairs(Chests) do
...
RegisterCommand("debugtarget", function()
    ...
    if exports["target"] then
        ...
        -- Tentar criar um target temporário para testar
        ...
        print("✅ Target de teste criado")
        
        -- Remover após 5 segundos
        SetTimeout(5000, function()
            exports["target"]:RemoveZone("TESTE_PRF")
            print("🗑️ Target de teste removido")
        end)
    else
...
AddEventHandler("teste:prf", function()
    print("🎯 Target de teste funcionando!")
end)